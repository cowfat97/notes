# how 的博客

Java 后端 & 大模型开发学习笔记。

线上阅读：[cowfat97.github.io/notes](https://cowfat97.github.io/notes/)

## 模块导航

| 模块 | 位置 | 说明 |
|------|------|------|
| Java | [Java/](./Java/) | 基础 / 多线程 / JVM / 框架 / 集合 |
| 中间件 | [中间件/](./中间件/) | Redis / Nginx / Kafka / ES / Docker / Nacos / PostgreSQL |
| 数据库 | [数据库/](./数据库/) | MySQL 索引 / 事务 / 特性 |
| LLM | [LLM/](./LLM/) | Agent / RAG / 深度学习 / 微调 / LangGraph |
| 408 | [408/](./408/) | 数据结构 / 计组 / 操作系统 / 计网 |
| LeetCode | [leetcode/](./leetcode/) | Python 刷题 |
| 软考 | [软考/](./软考/) | 软件设计师备考 |

## Java

- [Java 的特性](Java/Java%20基础/Java%20的特性.md)
- [HashMap 线程安全吗？](Java/集合/HashMap%20线程安全吗？.md)
- [List](Java/集合/List.md)
- [线程创建](Java/多线程/线程创建.md)
- [线程状态](Java/多线程/线程状态.md)
- [JVM 内存结构](Java/JVM/JVM%20内存结构.md)
- [Spring — IoC 的实现](Java/框架/spring/Ioc%20的实现.md)
- [MyBatis — 适配器模式](Java/框架/mybatis/设计模式/适配器模式.md)
- [Druid](Java/工具/Druid.md)
- [Swagger](Java/工具/Swagger.md)

## 中间件

- [Redis 集群模式](中间件/Redis/Redis集群模式.md)
- [Nginx 配置](中间件/Nginx/Nginx%20配置.md)
- [Elasticsearch](中间件/Elasticsearch/)
- [Kafka](中间件/Kafka/)
- [Docker](中间件/Docker/)
- [Nacos](中间件/Nacos/)
- [PostgreSQL](中间件/PostgreSQL/)

## 数据库

- [索引](数据库/Mysql/索引.md)
- [数据库事务](数据库/Mysql/数据库事务.md)
- [数据库特性](数据库/Mysql/数据库特性.md)

## LLM

- [Agent](LLM/Agent/) — Hello Agents / LangChain / LangGraph / MCP / A2A
- [RAG](LLM/Rag/) — RAG 概述 / 文档处理 / 文档向量化
- [深度学习](LLM/深度学习/)
- [微调](LLM/微调/)
- [费曼视频](LLM/费曼视频.md)

## 408 考研

- [数据结构](408/数据结构/)
- [计算机组成原理](408/计算机组成原理/)
- [操作系统](408/操作系统/)
- [计算机网络](408/计算机网络/)

## 笔记规范

- 中文书写，Markdown 格式
- 概念 + 实战示例，结构清晰便于复习
