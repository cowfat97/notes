...
1. **知识截止** — 训练数据有截止日期，无法获取实时信息（天气、股价、新闻）
2. **无外部行动能力** — 只能生成文本，不能执行实际操作（查数据库、发邮件、调用 API）
3. **幻觉问题** — 不知道的事情会"编造"，而不是说"我不知道"或主动去查

## Agent 解决的核心问题

让 LLM 从"只会说"变成"会说也会做"——能够主动调用外部工具获取信息、执行操作，弥补 LLM 自身的能力边界。
...
> **Agent = 大模型 + 规划(Planning) + 工具(Tools) + 记忆(Memory)**
...
| Planning | 任务分解与规划 | ReAct / 规划模式 |
| Tools | 外部能力扩展 | Function Call / MCP |
| Memory | 上下文与历史记录 | 对话历史 / 向量数据库 |
...
| 层级 | 技术 | 解决问题 |
|------|------|----------|
| 工具调用 | Function Call | 让 LLM 能调用外部函数 |
| 工具标准化 | MCP | 统一工具接入接口，跨平台复用 |
| 智能决策 | Agent 五种模式 | 复杂任务的推理、规划、执行 |
| Agent 间协作 | A2A | Agent 到 Agent 的标准化通信 |
| 框架 | LangChain / LangGraph | 提供 Agent 开发抽象和编排能力 |
